...
fn main() {
    ...
    for name in names {
        ...
        for _ in 0..n {
            ...
            let brute: Option<f32> = b
                .scene
                .shapes
                .iter()
                .filter_map(|s| s.hit(&ray, 1e-3, 1e9))
                .map(|h| h.t)
                .min_by(|a, c| a.partial_cmp(c).unwrap());
            let bvh = b
                .scene
                .bvh
                .intersect(&b.scene.shapes, &ray, 1e-3, 1e9)
                .map(|(h, _)| h.t);
            ...
            let b_occ = b
                ...
                .shapes
                .iter()
                .any(|s| s.hit(&ray, 1e-3, t_max).is_some());
